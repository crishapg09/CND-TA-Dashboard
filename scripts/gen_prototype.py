...
def render_demand(rows):
    ...
    return f'''{panelhead('Status of TA', 'The full lifecycle of nutrition TA requests — received, in progress, completed and overdue.')}
      ...
      <div class="panelhead" style="margin:0 0 14px">
        ...
        <div class="ps">The charts below show how fast the standing portfolio of TA requests is building, and in which stage the work gets stuck.</div>
      </div>
      <div class="card">
...
def render_work(rows):
    ...
    LEAD_C, COLLAB_C, EXT_C = '#0B5A8A', '#5FB49C', '#C08636'
    inv = {}
    ...
    def _inv_row(nm, v, both=True):
        lead, col = v['lead'], v['collab']
        t = lead + col
        tl = esc(f'{nm} — leads {lead} request{_pl(lead)}')
        tc = esc(f'{nm} — collaborates on {col} request{_pl(col)}')
        if both:
            w = t
            inner = ''
            if lead:
                inner += f'<span title="{tl}" style="width:{100*lead/t:.2f}%;background:{LEAD_C}"></span>'
            if col:
                inner += f'<span title="{tc}" style="width:{100*col/t:.2f}%;background:{COLLAB_C}"></span>'
            n, split = t, f'<div class="invsplit" style="color:{COLLAB_C}">{round(100*col/t) if t else 0}%</div>'
        else:
            w = lead
            inner = f'<span title="{tl}" style="width:100%;background:{LEAD_C}"></span>' if lead else ''
            n, split = lead, '<div class="invsplit" style="color:#B9C3CC">lead</div>'
        return (f'<div class="invrow"><div class="invlabel"><div class="invname">{esc(nm)}</div><div class="invarea">{esc(v["area"])}</div></div>'
                f'<div class="track" style="height:12px;background:#EEF2F6;border-radius:6px"><div class="bar" style="height:100%;width:{100*w/pmax_t:.1f}%;border-radius:6px">{inner}</div></div>'
    ...
    def _area_pane(mode):
        ...
        for a, (l, i, e) in area_sorted:
            ...
            if mode == 'lead':
                ...
                if i:
                    inner += f'<span title="{ti}" style="width:{100*i/t:.2f}%;background:{COLLAB_C}"></span>'
                if e:
                    inner += f'<span title="{te}" style="width:{100*e/t:.2f}%;background:{EXT_C}"></span>'
            html += (f'<div class="invrow" style="grid-template-columns:230px 1fr 40px 62px">'
                     f'<div class="invlabel"><div class="invname">{esc(a)}</div></div>'
                     f'<div class="track" style="height:13px;background:#EEF2F6;border-radius:6px"><div class="bar" style="height:100%;width:{barw:.1f}%;border-radius:6px">{inner}</div></div>'
                     f'<div class="invn">{num}</div>{split}</div>')
        return html
    area_all, area_lead = _area_pane('all'), _area_pane('lead')
    ...
    area_legend = (f'<div class="legend"><div class="lg"><span class="lgdot" style="background:{LEAD_C}"></span>Lead (Assigned to)</div>'
                   f'<div class="lg"><span class="lgdot" style="background:{COLLAB_C}"></span>Internal collaborators</div>'
    ...
    def _busy_bar(nm, lrows, collab):
        ...
        for st in STATUS_ORDER:
            ...
            if w:
                tt = esc(f'{nm} — {w} led request{_pl(w)} at {st}')
                inner += f'<span title="{tt}" style="width:{100*w/tot:.2f}%;background:{SC[st]}"></span>'
        if collab:
            tt = esc(f'{nm} — collaborates on {collab} more request{_pl(collab)}')
            inner += f'<span title="{tt}" style="width:{100*collab/tot:.2f}%;background:{COLLAB_C}"></span>'
    ...
    def _busy_pane(mode):
        ...
        for name, lrows in lead_groups:
            ...
            if mode == 'lead':
                bar, extra = _busy_bar(name, lrows, 0), ''
            else:
                bar = _busy_bar(name, lrows, col)
                extra = f'<span style="color:{COLLAB_C};font-weight:700"> +{col}</span>' if col else ''
            html += (f'<div class="leadrow" style="grid-template-columns:180px 1fr 58px"><div class="leadlabel"><div class="leadname">{esc(name)}</div>'
                     f'<div class="leadarea">{esc(lrows[0]["area"])}</div></div>'
                     f'{bar}<div class="ln">{len(lrows)}{extra}</div></div>')
        return html
    busy_all, busy_lead = _busy_pane('all'), _busy_pane('lead')
    ...
    def _spread_block(vals, name_max, per_label, tag, shown):
        ...
        return (f'<div class="modepane" data-mode="{tag}" style="display:{disp}">'
    ...
    spread_all = _spread_block(team_totals or [0], team_max_nm, 'nutrition-team staff involved (lead + collaborator)', 'all', True)
    spread_lead = _spread_block(counts, lead_groups[0][0], 'TA leads assigned', 'lead', False)
    ...
    return f'''{head}
      ...
      <div class="card mt16">
        ...
        <div class="modepane" data-mode="lead" style="display:none"><div class="leadgrid">{busy_lead}</div></div>
      </div>'''
...
<style>
  * {{ box-sizing:border-box; }}
  ...
  /* programme offers — full width, large labels, icon per offer */
  .offerrow {{ display:grid; grid-template-columns:36px 360px 1fr 40px; gap:16px; align-items:center; margin-bottom:13px; }}
  .offericon {{ width:32px; height:32px; border-radius:9px; background:#EAF2F8; display:flex; align-items:center; justify-content:center; font-size:17px; line-height:1; }}
  .offerlbl {{ font-size:14px; color:#0F2238; font-weight:600; line-height:1.3; }}
  .offerrow .bln {{ font-size:14px; }}
  @media (max-width:760px) {{ .offerrow {{ grid-template-columns:36px 1fr 40px; }} .offerlbl {{ grid-column:2 / 4; }} }}
  /* collapsible TA leads inside the thematic-areas card */
  .cleads {{ margin-top:16px; border-top:1px solid #F1F4F7; padding-top:6px; }}
  .cleadsum {{ cursor:pointer; list-style:none; font-size:13px; font-weight:700; color:#0B5A8A; padding:7px 2px; display:flex; align-items:center; gap:9px; }}
  .cleadsum::-webkit-details-marker {{ display:none; }}
  .cleads .cchev {{ display:inline-block; transition:transform .15s ease; color:#9AA7B2; font-size:11px; }}
  ...
  /* thematic area -> assigned lead staff tree (collapsible per area) */
  .taarea {{ border-top:1px solid #F1F4F7; }}
  .taarea:first-child {{ border-top:none; }}
  .taarea-head {{ display:grid; grid-template-columns:20px 240px 1fr 40px; gap:16px; align-items:center; cursor:pointer; list-style:none; padding:12px 0; }}
  .taarea-head::-webkit-details-marker {{ display:none; }}
  .taarea-head:hover .taarea-name {{ color:#0B5A8A; }}
  .taarea-name {{ font-size:13.5px; font-weight:700; color:#0F2238; }}
  .taarea-head .bln {{ font-size:13.5px; }}
  .tachev {{ color:#9AA7B2; font-size:11px; transition:transform .15s ease; }}
  .taarea[open] .tachev {{ transform:rotate(90deg); }}
  .taleads {{ margin:2px 0 14px 34px; padding-left:16px; border-left:2px solid #EDF1F4; }}
  ...
  @media (max-width:680px) {{ .taarea-head {{ grid-template-columns:20px 1fr 60px 40px; }} .talead {{ grid-template-columns:1fr 50px 34px; }} }}
</style></head>
<body>
<div class="wrap">
...
function showMode(m){{
  var ps=document.querySelectorAll('.modepane');
  for(var i=0;i<ps.length;i++){{ ps[i].style.display = ps[i].getAttribute('data-mode')===m ? '' : 'none'; }}
  var ts=document.querySelectorAll('.modetoggle');
  for(var j=0;j<ts.length;j++){{ if(ts[j].getAttribute('data-mode')===m){{ ts[j].classList.add('on'); }} else {{ ts[j].classList.remove('on'); }} }}
}}
function sortDet(el){{
...
var OFFER_ICON={{
  'Programme Management Scale-Up Support':'\\u{{1F4C8}}',
  'Policy Reform and Programme Design':'\\u{{1F4CB}}',
  'Financing for Scale':'\\u{{1F4B0}}',
  'Humanitarian Action and Resilience':'\\u{{26D1}}\\u{{FE0F}}',
  'Digital and Infrastructure':'\\u{{1F5A5}}\\u{{FE0F}}',
  'Workforce Development and Institution Building':'\\u{{1F3DB}}\\u{{FE0F}}',
  'Equity and Demand Generation':'\\u{{1F91D}}'
}};
function _offerBars(items){{
  if(!items||!items.length){{ return '<div class="muted">None recorded.</div>'; }}
  var mx=1,i; for(i=0;i<items.length;i++){{ mx=Math.max(mx,items[i][1]); }}
  var h='';
  for(i=0;i<items.length;i++){{
    var nm=items[i][0], n=items[i][1], ic=OFFER_ICON[nm]||'\\u{{1F539}}';
    h+='<div class="offerrow">'
      +'<div class="offericon">'+ic+'</div>'
      +'<div class="offerlbl">'+_cesc(nm)+'</div>'
      +'<div class="track" style="height:11px;background:#E9F0F6;border-radius:6px"><div style="height:100%;width:'+(100*n/mx).toFixed(1)+'%;background:#0B6FA4;border-radius:6px"></div></div>'
...
function _areaTree(areas){{
  ...
  for(i=0;i<areas.length;i++){{
    ...
    h+='<details class="taarea"><summary class="taarea-head"><span class="tachev">&#9656;</span><div class="taarea-name">'+_cesc(nm)+'</div>'
